...
export const COMMAND: Command = {
  ...
  run: async (client, message, args) => {
    try {
      ...
      if(player.queue.current.isStream) return await message.reply(`${client.emotes.x} **你不能快轉直播!**`);
      if(!seconds || !Number.isInteger(seconds)) return await message.reply(`${client.emotes.x} **無效的秒數!**`);
      const current = player.position;
      const target = ((current + (seconds * 1000)) >= player.queue.current.duration! ? player.queue.current.duration! : current + (seconds * 1000));
      player.seek(target);
      await message.reply(`${client.emotes.v} **將音樂快轉到** \`${lengthFormmater(target)}\``);
    } catch(err) {
